...
/**
 * Copies exactly `n` bytes from `src` (at srcOffset) into `dst` (at dstOffset).
 *
 ...
 * @throws {TypeError}  On invalid argument types.
 * @throws {RangeError} If buffer is too small for the encoded message.
 */
function copyMessageToBuffer(buffer, message) {
  ...
  const srcBytes = encoder.encode(message); // UTF-8, never truncates
