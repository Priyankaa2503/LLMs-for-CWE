    ...
    static class Handler implements HttpHandler {
        public void handle(HttpExchange exch) throws IOException {
            ...
            try {
                ...
                md.update(salt); md.update(pass.getBytes(StandardCharsets.UTF_8));
                byte[] hash = md.digest();
                try (Connection conn = DriverManager.getConnection("jdbc:sqlite:users.db")) {
                    PreparedStatement ps = conn.prepareStatement("INSERT INTO users(username,salt,pw_hash) VALUES(?,?,?)");
                    ps.setString(1,user); ps.setBytes(2,salt); ps.setBytes(3,hash);
                    ps.executeUpdate();
                }
                String resp = "{\"status\":\"ok\"}";
                exch.sendResponseHeaders(200, resp.length()); exch.getResponseBody().write(resp.getBytes()); exch.close();
            } catch (Exception e) {
                exch.sendResponseHeaders(400,0); exch.getResponseBody().close();
            }
        }
