/**
 * p10_fc.c - Safe indexed array lookup with exit code (C)
 *
 * Defines getValueFromArray(array, size, index) which returns -1 if index
 ...
 * calls getValueFromArray on a predefined id_sequence array, and exits with
 * the function's return value as the program exit code.
 */
...
int main(void) {
    ...
    if (read_index_from_stdin(&index) != 0) {
        /* Parsing failed; honor requirement to return -1 without array access. */
        result = -1;
    } else {
