/*
 * p10_nc.c - Safe indexed lookup with exit code (C)
 *
 ...
 * reads an integer index from standard input, validates it, and returns the
 * function's result as the process exit code without causing undefined
 * behavior.
 */
...
int main(void) {
    ...
    /* getValueFromArray performs its own bounds checks to avoid OOB access. */
