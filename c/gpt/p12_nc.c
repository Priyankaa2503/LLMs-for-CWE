/*
 * p12_nc.c - Safe indexed lookup with strict bounds enforcement (C)
 *
 * Implements getValueFromArray(array, size, index) that returns array[index]
 * when 0 <= index < size and returns -1 otherwise. The main function reads an
 * integer index from standard input, performs safe parsing and validation, and
 * returns the function's result as the program's exit code without causing
 * undefined behavior.
 */
