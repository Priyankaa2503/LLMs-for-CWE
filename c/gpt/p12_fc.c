/**
 * p12_fc.c - Safe array access with bounds-checked helper (C)
 *
 * Defines getValueFromArray(array, size, index) which returns the element
 * at index when 0 <= index < size and returns -1 otherwise. The main
 * function reads the index from standard input, safely parses it, calls
 * getValueFromArray with a predefined id_sequence and its size, and exits
 * with the function's return value as the program's exit code.
 */
...
static const int ID_SEQUENCE[] = {7, 14, 21, 28, 35};
