...
int safe_memcpy_string(char *dest, size_t dest_capacity, const char *src) {
    ...
    size_t len = strnlen(src, dest_capacity); // bounded — never overreads past cap
    if (len >= dest_capacity) {               // >= to reserve one byte for '\0'
        fprintf(stderr, "Error: destination buffer too small.\n");
        return -1;
    }

    memcpy(dest, src, len + 1);  // +1 copies the null terminator safely
    return 0;
}
